...
Handlebars.registerHelper('add', (...args: any[]) => {
	...
	const numbers = args.slice(0, -1).map(n => Number(n)).filter(n => !isNaN(n));
...
export function createTemplateContext(el: HTMLElement, ctx: MarkdownPostProcessorContext, baseView: any): TemplateContext {
